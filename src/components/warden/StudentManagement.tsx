...
const StudentManagement = () => {
  ...
  <Input
    ...
    onChange={(e) => {
  // keep digits only and cap to 10 chars
      const digits = e.target.value.replace(/\D/g, '').slice(0, 10);
      setNewStudent({ ...newStudent, guardian_phone: digits });
    }}
  />

  <p className={`text-sm mt-1 ${newStudent.guardian_phone.length === 10 ? 'text-green-600' : 'text-destructive'}`}>
    {newStudent.guardian_phone.length === 0
      ? 'Enter 10-digit phone number'
      : newStudent.guardian_phone.length === 10
      ? 'Valid phone number'
