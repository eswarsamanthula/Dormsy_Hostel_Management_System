...
const Auth = () => {
  ...
  // Password rules state (fixed: previously missing)
  ...
  const allSignUpRulesValid = Object.values(passwordRules).every(Boolean);
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    // Re-validate rules from current input (defensive)
    const currentRules = validatePassword(signUpData.password);
    setPasswordRules(currentRules);

    if (!Object.values(currentRules).every(Boolean)) {
      setError('Password does not meet the required complexity. Minimum 8 chars, uppercase, number, special char.');
      setIsLoading(false);
      return;
    }
              ...
              <TabsContent value="signup" className="space-y-4">
                <form onSubmit={handleSignUp} className="space-y-4">
                  ...
                  <Button
                    ...
                    className="w-full"
                    variant="hero"
                    disabled={isLoading || !allSignUpRulesValid}
                    title={!allSignUpRulesValid ? "Password doesn't meet complexity requirements" : undefined}
                  >
