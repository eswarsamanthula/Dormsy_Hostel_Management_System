...
-- Safety Rules
((SELECT id FROM hostels LIMIT 1), (SELECT id FROM colleges LIMIT 1), 'Fire Safety', 'Use of heating appliances, candles, or incense sticks is strictly prohibited. Fire extinguishers are located on each floor - do not tamper with them.', 'safety', true),
((SELECT id FROM hostels LIMIT 1), (SELECT id FROM colleges LIMIT 1), 'Electrical Safety', 'Do not overload electrical sockets. Report any electrical faults immediately to the maintenance team. Use of personal electrical appliances requires approval.', 'safety', true),
((SELECT id FROM hostels LIMIT 1), (SELECT id FROM colleges LIMIT 1), 'Emergency Procedures', 'In case of emergency, contact the warden immediately. Emergency contact numbers are displayed on each floor. Follow evacuation procedures during fire drills.', 'safety', true),

-- Conduct Rules
((SELECT id FROM hostels LIMIT 1), (SELECT id FROM colleges LIMIT 1), 'Noise Policy', 'Maintain silence after 10:00 PM. Loud music, parties, or gatherings that disturb others are not allowed. Use headphones for entertainment.', 'conduct', true),
((SELECT id FROM hostels LIMIT 1), (SELECT id FROM colleges LIMIT 1), 'Alcohol and Smoking', 'Consumption of alcohol, smoking, and use of tobacco products is strictly prohibited within hostel premises. Violation will result in disciplinary action.', 'conduct', true),
((SELECT id FROM hostels LIMIT 1), (SELECT id FROM colleges LIMIT 1), 'Ragging Policy', 'Ragging in any form is strictly prohibited and is a punishable offense. Report any incidents immediately to the authorities.', 'conduct', true),

-- Mess Rules
((SELECT id FROM hostels LIMIT 1), (SELECT id FROM colleges LIMIT 1), 'Mess Timings', 'Breakfast: 7:00-9:00 AM, Lunch: 12:00-2:00 PM, Dinner: 7:00-9:00 PM. Late comers will not be served food.', 'mess', true),
((SELECT id FROM hostels LIMIT 1), (SELECT id FROM colleges LIMIT 1), 'Mess Etiquette', 'Maintain cleanliness in the dining hall. Do not waste food. Return plates and utensils to designated areas after eating.', 'mess', true),
((SELECT id FROM hostels LIMIT 1), (SELECT id FROM colleges LIMIT 1), 'Outside Food', 'Outside food delivery is allowed only until 9:00 PM. Food should not be consumed in rooms - use common dining areas.', 'mess', true);